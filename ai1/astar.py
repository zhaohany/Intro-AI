...
def neibor_check(map,p,gx,gy):
    ...
    right_y=y+1
    g=map[x][y][3]
    if -1< up_x <= 101 and -1< up_y <=101 and map[up_x][up_y][0]!= 1 and map[up_x][up_y][1] != 1:
        map[up_x][up_y][2]=g+manhadun(up_x,up_y,gx,gy)
        map[up_x][up_y][3]=g+1
        cango.append(map[up_x][up_y])
        # print(cango)

    if -1< down_x <= 101 and -1< down_y <=101 and map[down_x][down_y][0]!= 1 and map[down_x][down_y][1] != 1:
        map[down_x][down_y][2]=g+manhadun(down_x,down_y,gx,gy)
        map[down_x][down_y][3]=g+1
        cango.append(map[down_x][down_y])
        # print(cango)
    if -1< left_x <= 101 and -1< left_y <=101 and map[left_x][left_y][0]!= 1 and map[left_x][left_y][1] != 1:
        map[left_x][left_y][2]=g+manhadun(left_x,left_y,gx,gy)
        map[left_x][left_y][3]=g+1
        cango.append(map[left_x][left_y])
        # print(cango)
    if -1< right_x <= 101 and -1< right_y <=101 and map[right_x][right_y][0]!= 1 and map[right_x][right_y][1] != 1:
...
for x in range(0,101):
    for y in range(0,101):
        ...
        if random.randint(1,10) <=7:
            ...
            map[x][y][0]=1
starx=random.randint(1,100)
stary=random.randint(1,100)
goalx=random.randint(1,100)
goaly=random.randint(1,100)
